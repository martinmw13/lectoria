...
**Page** — A read-time display unit produced by paginating a Scene at paragraph boundaries (≤~250 words per page; D19). A Scene decomposes into one or more Pages that never cross scene boundaries; each Page knows its position (`isFirstPage`/`isLastPage`, `pageInScene`/`totalPagesInScene`). The reader renders one Page at a time (`PageView`). **Scene** remains the narrative-segmentation and music-sync unit; **Page** is purely presentational. Traversal over Pages is owned by the **ReaderCursor**. Logic: `frontend/src/utils/paginate.ts`.

**ReaderCursor** — The pure, React-free traversal over the NCM + source paragraphs that owns "where am I, what Page renders, which Scene drives music, how do I move." Given an `NCM`, `ChaptersData`, and a `{chapterIdx, pageIdx}` position it answers `currentPage`, `musicScene`, and `prevMusicScene`, and produces a new cursor on `next()`/`prev()`/`goToChapter(i)` (or `null` at a no-move boundary). Pagination (D19) and the cross-chapter music-scene derivation (D7, D12) live here, unit-tested without a render; the ~280 ms slide animation stays in `ReaderPage`. Held via the thin `useReaderCursor` hook — the same framework-free-core + thin-hook split as the **CrossfadeAudioPlayer** (ADR-0002). Logic: `frontend/src/reader/cursor.ts`.
